...
class Mechanic {
  ...
  func performJob(_ job: Job) -> Bool {
    if job.minimumSkillSet > self.skill || isBusy == true {
      assert(false, "This mechanic is either busy or insufficiently skilled for this job, " +
        "he should have never been asked to perform it, " +
        "there is something wrong in the chain of responsibility")
    } else {
      isBusy = true
